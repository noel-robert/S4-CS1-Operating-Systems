...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (scanf("%d", &process[i].burstTime) > 0) {} else {printf("Error in inputting");return -1;}
		...
		printf("Input priority for P%d ", i + 1);
    if (scanf("%d", &process[i].priority) > 0) {} else {printf("Error in inputting");return -1;}
  ...
  printf("P[]\t\tAT\tBT\n");
  for (int i = 0; i < n; i++) {
    printf("P[%d]\t%d\t%d\t%d\n", process[i].pno, process[i].arrivalTime, process[i].burstTime, process[i].priority);
  }
  printf("\n");
  ...
  for (int i=0; i<n; i++) {
    for (int j=i+1; j<n; j++) {
      if ((process[i].arrivalTime>process[j].arrivalTime) || 
				((process[i].arrivalTime==process[j].arrivalTime)&&(process[i].priority>process[j].priority))) {
  ...
  printf("Output Data\n");
  printf("P[]\t\tTAT\tWT\tPriority\n");
  for (int i = 0; i < n; i++) {
    printf("P[%d]\t%d\t%d\t%d\n", process[i].pno, process[i].turnAroundTime,
           process[i].waitingTime, process[i].priority);
  }
  printf("\n");
